...
### 更新其他实例（Instance2、Instance3...）

#### 方式一：使用管理脚本（推荐）

```bash
cd ~/score-system
./manage-instances.sh update in stance2
```
...
rsync -av \
  ...
  --exclude='*.backup' \
  --exclude='update.sh' \
  ~/score-system/ ~/score-system-instance2/
...
在 Instance2 目录创建更新脚本（只需创建一次）：

```bash
cat > ~/score-system-instance2/update.sh << 'EOF'
#!/bin/bash
echo "========================================="
echo "  更新 Instance2 代码"
echo "========================================="
...
chmod +x ~/score-system-instance2/update.sh
```
...
```bash
# 更新主系统
cd ~/score-system && git pull && docker-compose build --no-cache && docker-compose up -d

# 更新 Instance2
~/score-system-instance2/update.sh
```
