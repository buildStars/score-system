# 数据源配置迁移说明 🔧

## 修改概述

将"开奖数据源"配置从前端管理界面移除，改为由后端统一管理。这样可以：
- ✅ 提高安全性（避免误修改数据源）
- ✅ 简化前端界面
- ✅ 集中管理配置

## 修改详情

### 1. 前端修改

**文件：** `frontend-admin/src/views/SystemSettings.vue`

#### 1.1 删除 UI 部分
删除了"数据源设置"整个区块：

```vue
<!-- 已删除 -->
<el-divider content-position="left">数据源设置</el-divider>

<el-form-item label="开奖数据源" prop="lotteryDataSource">
  <el-input
    v-model="form.lotteryDataSource"
    placeholder="请输入开奖数据源URL"
  />
  <span class="form-tip">开奖数据获取接口地址</span>
</el-form-item>
```

#### 1.2 删除验证规则
```typescript
// 已删除
lotteryDataSource: [
  { required: true, message: '请输入开奖数据源URL', trigger: 'blur' },
  { type: 'url' as const, message: '请输入有效的URL', trigger: 'blur' },
],
```

#### 1.3 提交时过滤字段
```typescript
// 提交设置
const handleSubmit = async () => {
  if (!formRef.value) return

  try {
    await formRef.value.validate()
    submitLoading.value = true
    
    // 移除 lotteryDataSource，由后端配置文件管理
    const { lotteryDataSource, ...settingsToUpdate } = form
    await updateSystemSettings(settingsToUpdate)
    
    ElMessage.success('保存成功')
  } catch (error) {
    console.error('保存设置失败:', error)
  } finally {
    submitLoading.value = false
  }
}
```

### 2. 后端修改

**文件：** `backend/src/modules/system/dto/update-system-settings.dto.ts`

标记 `lotteryDataSource` 为已废弃：

```typescript
@ApiProperty({ description: '开奖数据源（已废弃，由后端配置文件管理）', required: false })
@IsOptional()
@IsString()
lotteryDataSource?: string;
```

**文件：** `backend/src/modules/system/system.service.ts`

将 `lotteryDataSource` 加入废弃字段列表：

```typescript
// 过滤掉已废弃的字段（由后端配置文件或其他方式管理）
const deprecatedFields = ['warningTime', 'lotteryDataSource'];

for (const [key, value] of Object.entries(updateDto)) {
  if (value !== undefined && !deprecatedFields.includes(key)) {
    // 只保存非废弃字段
  }
}
```

### 3. 后端配置管理

数据源现在由后端配置文件管理，在以下位置配置：

**文件：** `backend/.env`

```env
# USA28 API 数据源
LOTTERY_DATA_SOURCE=https://api.365kaik.com/api/v1/trend/getHistoryList
```

或者在数据库的 `system_settings` 表中手动维护：

```sql
-- 查看当前数据源配置
SELECT * FROM system_settings WHERE setting_key = 'lottery_data_source';

-- 更新数据源（如需要）
UPDATE system_settings 
SET setting_value = 'https://api.365kaik.com/api/v1/trend/getHistoryList' 
WHERE setting_key = 'lottery_data_source';
```

## 配置优先级

后端获取数据源的优先级：
1. **环境变量** `.env` 文件中的 `LOTTERY_DATA_SOURCE`
2. **数据库配置** `system_settings` 表中的 `lottery_data_source`
3. **默认值** 硬编码的默认值

## 前端界面变化

### 修改前
```
系统设置
├── 游戏控制
│   ├── 游戏开关
│   ├── 维护模式
│   └── 自动结算
├── 封盘时间设置
│   ├── 开奖间隔时间
│   └── 封盘时间
├── 系统公告
│   └── 系统公告内容
└── 数据源设置 ❌ 已删除
    └── 开奖数据源
```

### 修改后
```
系统设置
├── 游戏控制
│   ├── 游戏开关
│   ├── 维护模式
│   └── 自动结算
├── 封盘时间设置
│   ├── 开奖间隔时间
│   └── 封盘时间
└── 系统公告
    └── 系统公告内容
```

## 测试验证

1. **前端页面**
   - ✅ 访问 `http://localhost:5174/system-settings`
   - ✅ 确认不再显示"数据源设置"区块
   - ✅ 其他配置可以正常保存

2. **后端数据同步**
   - ✅ 检查开奖数据同步是否正常工作
   - ✅ 数据源应从 `.env` 或数据库读取
   - ✅ 不受前端设置页面影响

3. **修改数据源**（如需要）
   ```bash
   # 方式1: 修改 .env 文件
   cd score-system/backend
   vim .env
   # 添加或修改: LOTTERY_DATA_SOURCE=https://api.xxx.com/...
   
   # 方式2: 直接修改数据库
   mysql -u root -p score_system
   UPDATE system_settings 
   SET setting_value = 'https://new-api.com/...' 
   WHERE setting_key = 'lottery_data_source';
   ```

## 兼容性说明

- ✅ 数据库中的 `lottery_data_source` 记录保留
- ✅ 后端仍会读取这个配置
- ✅ 只是前端不再提供编辑界面
- ✅ 如果前端误发送，后端会自动过滤

## 完成状态

- ✅ 前端页面删除"数据源设置"
- ✅ 前端提交时过滤 `lotteryDataSource`
- ✅ 后端 DTO 标记字段为废弃
- ✅ 后端 Service 过滤废弃字段
- ✅ 代码 Linter 检查通过

## 回滚方案

如果需要恢复前端配置界面：

1. 恢复 `SystemSettings.vue` 中的"数据源设置"区块
2. 恢复验证规则
3. 修改提交逻辑，不再过滤 `lotteryDataSource`
4. 从后端 `deprecatedFields` 中移除 `lotteryDataSource`

---

**更新时间：** 2025-11-27
**相关文档：**
- warningTime字段移除说明.md
- 删除退水功能说明.md





