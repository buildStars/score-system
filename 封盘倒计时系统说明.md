...
sequenceDiagram
    ...
    Backend->>Backend: 启动时初始化
    Backend->>USA28: 获取最新开奖
    USA28-->>Backend: 返回最新期号+时间
    Backend->>Backend: 缓存数据

    User->>Frontend: 打开页面
    Frontend->>Backend: GET /lottery/status
    Backend->>Backend: 计算倒计时和状态
    Backend-->>Frontend: 返回完整状态
    Frontend->>Frontend: 开始本地倒计时

    loop 每秒
        Frontend->>Frontend: localCountdown--
    end

    Frontend->>Backend: 倒计时结束，重新同步
...
sequenceDiagram
    ...
    User->>Frontend: 点击下注
    Frontend->>Frontend: 检查本地状态
    
    alt 已封盘（前端判断）
        Frontend-->>User: 提示：本期已封盘
    else 开盘中
        Frontend->>Backend: POST /bet/create
        Backend->>Backend: canPlaceBet()
        
        alt 封盘期间（后端判断）
            Backend-->>Frontend: 400: 已封盘
            Frontend-->>User: 提示：封盘期间不可下注
        else 可以下注
            Backend->>Backend: 验证期号
            Backend->>DB: 创建下注记录
            Backend-->>Frontend: 200: 下注成功
            Frontend-->>User: 提示：下注成功
        end
    end
```
