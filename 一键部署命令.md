# 🚀 一键部署命令

本文档提供后端、H5前端、管理后台的一键部署命令，复制即可使用。

---

## 📦 1. 后端一键部署

### Docker 环境（推荐）

```bash
cd /path/to/score-system && \
docker-compose down && \
docker-compose build backend && \
docker-compose up -d && \
echo "✅ 后端部署完成！" && \
docker-compose logs -f backend
```

### 本地开发环境

```bash
cd /path/to/score-system/backend && \
npm run build && \
pm2 restart score-system-backend || npm run start:prod
```

---

## 📱 2. H5前端一键部署

### 方式1：Docker 部署（推荐）

```bash
cd /path/to/score-system && \
docker-compose down frontend-h5 && \
docker-compose build frontend-h5 && \
docker-compose up -d frontend-h5 && \
echo "✅ H5前端部署完成！访问地址：http://your-domain:8080" && \
docker-compose logs -f frontend-h5
```

### 方式2：静态文件部署（Nginx）

```bash
cd /path/to/score-system/frontend-h5 && \
npm run build && \
rm -rf /var/www/h5/* && \
cp -r dist/* /var/www/h5/ && \
nginx -s reload && \
echo "✅ H5前端部署完成！"
```

---

## 💼 3. 管理后台一键部署

### 方式1：Docker 部署（推荐）

```bash
cd /path/to/score-system && \
docker-compose down frontend-admin && \
docker-compose build frontend-admin && \
docker-compose up -d frontend-admin && \
echo "✅ 管理后台部署完成！访问地址：http://your-domain:8081" && \
docker-compose logs -f frontend-admin
```

### 方式2：静态文件部署（Nginx）

```bash
cd /path/to/score-system/frontend-admin && \
npm run build && \
rm -rf /var/www/admin/* && \
cp -r dist/* /var/www/admin/ && \
nginx -s reload && \
echo "✅ 管理后台部署完成！"
```

---

## 🎯 4. 全栈一键部署

如果需要**同时部署所有服务**：

```bash
cd /path/to/score-system && \
docker-compose down && \
docker-compose build && \
docker-compose up -d && \
echo "✅ 全栈部署完成！" && \
docker-compose ps && \
echo "\n📋 查看后端日志：docker-compose logs -f backend" && \
echo "📋 查看H5日志：docker-compose logs -f frontend-h5" && \
echo "📋 查看管理后台日志：docker-compose logs -f frontend-admin"
```

---

## 🔍 5. 部署后验证

### 检查服务状态

```bash
# 查看所有服务状态
docker-compose ps

# 查看后端日志
docker-compose logs -f backend

# 查看H5前端日志
docker-compose logs -f frontend-h5

# 查看管理后台日志
docker-compose logs -f frontend-admin
```

### 检查端口

```bash
# 检查端口占用
netstat -tulpn | grep -E '3000|8080|8081|3306'

# 或使用 ss 命令
ss -tulpn | grep -E '3000|8080|8081|3306'
```

### 访问地址

- **后端API**：`http://your-domain:3000`
- **H5前端**：`http://your-domain:8080`
- **管理后台**：`http://your-domain:8081`
- **数据库**：`localhost:3306`（仅本地访问）

---

## ⚡ 6. 快速回滚（如果部署失败）

### 回滚后端

```bash
cd /path/to/score-system && \
git checkout HEAD~1 backend/ && \
docker-compose build backend && \
docker-compose up -d backend && \
echo "✅ 后端已回滚到上一版本！"
```

### 回滚H5前端

```bash
cd /path/to/score-system && \
git checkout HEAD~1 frontend-h5/ && \
docker-compose build frontend-h5 && \
docker-compose up -d frontend-h5 && \
echo "✅ H5前端已回滚到上一版本！"
```

### 回滚管理后台

```bash
cd /path/to/score-system && \
git checkout HEAD~1 frontend-admin/ && \
docker-compose build frontend-admin && \
docker-compose up -d frontend-admin && \
echo "✅ 管理后台已回滚到上一版本！"
```

---

## 📝 7. Windows PowerShell 命令

### 后端部署（Windows）

```powershell
cd D:\download\yunce\yunce\score-system; `
docker-compose down; `
docker-compose build backend; `
docker-compose up -d; `
Write-Host "✅ 后端部署完成！" -ForegroundColor Green; `
docker-compose logs -f backend
```

### H5前端部署（Windows）

```powershell
cd D:\download\yunce\yunce\score-system; `
docker-compose down frontend-h5; `
docker-compose build frontend-h5; `
docker-compose up -d frontend-h5; `
Write-Host "✅ H5前端部署完成！" -ForegroundColor Green; `
docker-compose logs -f frontend-h5
```

### 管理后台部署（Windows）

```powershell
cd D:\download\yunce\yunce\score-system; `
docker-compose down frontend-admin; `
docker-compose build frontend-admin; `
docker-compose up -d frontend-admin; `
Write-Host "✅ 管理后台部署完成！" -ForegroundColor Green; `
docker-compose logs -f frontend-admin
```

---

## 🛠️ 8. 常见问题排查

### 1. 端口被占用

```bash
# 查找占用端口的进程
lsof -i :3000  # 后端
lsof -i :8080  # H5前端
lsof -i :8081  # 管理后台

# 停止占用端口的进程
kill -9 <PID>
```

### 2. 容器启动失败

```bash
# 查看容器日志
docker-compose logs backend
docker-compose logs frontend-h5
docker-compose logs frontend-admin

# 重启特定服务
docker-compose restart backend
docker-compose restart frontend-h5
docker-compose restart frontend-admin
```

### 3. 清理并重新部署

```bash
# 清理所有容器、镜像、卷
cd /path/to/score-system && \
docker-compose down -v && \
docker system prune -f && \
docker-compose build --no-cache && \
docker-compose up -d && \
echo "✅ 清理并重新部署完成！"
```

---

## 📌 9. 部署前检查清单

- [ ] 代码已提交到 Git
- [ ] `.env` 配置文件已更新
- [ ] 数据库连接配置正确
- [ ] 防火墙已开放必要端口（3000, 8080, 8081）
- [ ] Docker 和 Docker Compose 已安装
- [ ] 磁盘空间充足（至少 5GB）
- [ ] 已备份数据库

---

## 🎉 10. 部署成功确认

部署完成后，访问以下地址确认：

1. **后端健康检查**：`http://your-domain:3000/api/health`
2. **H5前端**：`http://your-domain:8080`
3. **管理后台**：`http://your-domain:8081`

如果全部正常访问，说明部署成功！🎊





