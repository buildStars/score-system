# 🚀 多实例部署 - 快速开始

一分钟快速在服务器上部署第二套系统！

---

## 最简单的方式

### 步骤1: 赋予执行权限

```bash
cd /root/score-system
chmod +x create-instance.sh manage-instances.sh
```

### 步骤2: 创建新实例

```bash
# 创建实例2（自动配置所有端口）
./create-instance.sh instance2 "客户A系统"
```

**就这么简单！** 🎉

脚本会自动：
- ✅ 复制代码到新目录 `/root/score-system-instance2`
- ✅ 生成安全的随机密码
- ✅ 分配不同的端口（MySQL: 3308, Backend: 3001, H5: 5175, Admin: 5176）
- ✅ 启动 Docker 容器
- ✅ 初始化数据库

---

## 访问新实例

脚本执行完成后会显示访问地址，例如：

```
访问地址:
  - H5用户端: http://your-server-ip:5175
  - 管理后台: http://your-server-ip:5176
  - API文档:  http://your-server-ip:3001/api-docs

默认管理员账号:
  用户名: admin
  密码: admin123
```

直接用浏览器打开就可以使用了！

---

## 管理实例

### 查看所有实例

```bash
./manage-instances.sh list
```

输出示例：

```
[1] score-system
    描述: 默认实例
    端口: Backend=3000 H5=5173 Admin=5174

[2] score-system-instance2
    描述: 客户A系统
    端口: Backend=3001 H5=5175 Admin=5176

[3] score-system-instance3
    描述: 测试环境
    端口: Backend=3002 H5=5177 Admin=5178
```

### 查看运行状态

```bash
./manage-instances.sh status
```

### 查看日志

```bash
# 查看所有服务日志
./manage-instances.sh logs instance2

# 只看后端日志
./manage-instances.sh logs instance2 backend
```

### 重启实例

```bash
# 重启所有服务
./manage-instances.sh restart instance2

# 只重启后端
./manage-instances.sh restart instance2 backend
```

### 停止/启动实例

```bash
# 停止
./manage-instances.sh stop instance2

# 启动
./manage-instances.sh start instance2
```

---

## 常见使用场景

### 场景1: 为新客户快速部署

```bash
# 1. 创建新实例
./create-instance.sh client_abc "ABC公司系统"

# 2. 查看访问信息
./create-instance.sh info client_abc

# 3. 发送给客户
# H5: http://your-server:5175
# 管理后台: http://your-server:5176
# 账号: admin / admin123
```

### 场景2: 搭建测试环境

```bash
# 创建测试实例
./create-instance.sh test "测试环境"

# 测试完成后删除
cd /root/score-system-test
docker-compose down -v
cd /root
rm -rf score-system-test
```

### 场景3: 定期备份所有实例

```bash
# 手动备份
./manage-instances.sh backup-all

# 设置每天自动备份（添加到 crontab）
echo "0 2 * * * cd /root/score-system && ./manage-instances.sh backup-all" | crontab -
```

### 场景4: 批量更新所有实例

```bash
# 更新所有实例到最新版本
./manage-instances.sh update-all
```

---

## 端口分配参考

| 实例 | MySQL | Redis | Backend | H5 | Admin |
|------|-------|-------|---------|-----|-------|
| 默认 | 3307 | 6380 | 3000 | 5173 | 5174 |
| instance2 | 3308 | 6381 | 3001 | 5175 | 5176 |
| instance3 | 3309 | 6382 | 3002 | 5177 | 5178 |
| instance4 | 3310 | 6383 | 3003 | 5179 | 5180 |
| instance5 | 3311 | 6384 | 3004 | 5181 | 5182 |

**规律**：每个新实例端口依次递增

---

## 使用域名访问（可选）

如果不想用端口访问，可以配置域名：

### 1. 配置DNS解析

在域名控制台添加A记录：
- `client-a.yourdomain.com` → `服务器IP`
- `admin-a.yourdomain.com` → `服务器IP`

### 2. 配置Nginx

```bash
# 创建配置文件
sudo vim /etc/nginx/sites-available/score-client-a

# 添加内容：
server {
    listen 80;
    server_name client-a.yourdomain.com;
    
    location / {
        proxy_pass http://localhost:5175;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
    
    location /api {
        proxy_pass http://localhost:3001;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
}

server {
    listen 80;
    server_name admin-a.yourdomain.com;
    
    location / {
        proxy_pass http://localhost:5176;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
    
    location /api {
        proxy_pass http://localhost:3001;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
}

# 启用配置
sudo ln -s /etc/nginx/sites-available/score-client-a /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx
```

### 3. 配置HTTPS（推荐）

```bash
sudo apt install certbot python3-certbot-nginx
sudo certbot --nginx -d client-a.yourdomain.com -d admin-a.yourdomain.com
```

现在可以用域名访问了：
- https://client-a.yourdomain.com
- https://admin-a.yourdomain.com

---

## 常见问题

### Q: 创建实例失败怎么办？

**A**: 查看错误信息，通常是：

1. **端口被占用**：
```bash
# 检查端口
netstat -tlnp | grep :5175

# 修改端口（编辑 .env）
vim /root/score-system-instance2/.env
```

2. **Docker 没启动**：
```bash
sudo systemctl start docker
```

3. **内存不足**：
```bash
# 查看内存
free -h

# 清理Docker缓存
docker system prune -a
```

### Q: 如何查看数据库密码？

**A**: 查看实例的 `.env` 文件

```bash
cat /root/score-system-instance2/.env | grep PASSWORD
```

### Q: 如何连接到实例数据库？

**A**: 使用 MySQL 客户端

```bash
# 查看密码和端口
cd /root/score-system-instance2
source .env
echo "MySQL端口: $MYSQL_PORT"
echo "用户: $MYSQL_USER"
echo "密码: $MYSQL_PASSWORD"
echo "数据库: $MYSQL_DATABASE"

# 连接
mysql -h 127.0.0.1 -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD $MYSQL_DATABASE
```

或使用图形化工具（Navicat、DBeaver等），连接信息同上。

### Q: 服务器资源够吗？

**A**: 资源参考：

- **每个实例需要**：
  - CPU: 0.5-1核
  - 内存: 1-2GB
  - 磁盘: 5-10GB

- **建议配置**：
  - 2核4G: 最多2个实例
  - 4核8G: 最多4个实例
  - 8核16G: 6个以上实例

查看当前资源：

```bash
# CPU和内存
./manage-instances.sh status

# 磁盘
df -h
docker system df
```

---

## 完整命令速查表

```bash
# 创建实例
./create-instance.sh <实例名> "<描述>"

# 查看实例信息
./create-instance.sh info <实例名>

# 列出所有实例
./create-instance.sh list
./manage-instances.sh list

# 查看运行状态
./manage-instances.sh status

# 查看日志
./manage-instances.sh logs <实例名> [服务名]

# 重启
./manage-instances.sh restart <实例名> [服务名]

# 启动/停止
./manage-instances.sh start <实例名>
./manage-instances.sh stop <实例名>

# 更新
./manage-instances.sh update <实例名>
./manage-instances.sh update-all

# 备份
./manage-instances.sh backup <实例名>
./manage-instances.sh backup-all

# 进入容器
cd /root/score-system-<实例名>
docker exec -it score-<实例名>-backend sh
```

---

## 下一步

- 📖 详细配置：参考 `多实例部署指南.md`
- 🔧 手动部署：适合特殊需求的场景
- 🔐 安全加固：修改默认密码、配置防火墙
- 📊 监控告警：集成 Prometheus + Grafana
- 🔄 自动备份：设置定时任务

---

**祝您使用愉快！** 🎉

如有问题，请查看完整文档或联系技术支持。

