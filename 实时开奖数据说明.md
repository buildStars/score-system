# 🎰 实时开奖数据说明

## 📋 功能概述

系统已升级为**实时开奖数据模式**，开奖历史页面将直接从 USA28 API 获取最新数据，无需手动同步。

---

## ✨ 主要特性

### 1️⃣ 实时数据展示
- ✅ 页面加载时自动获取最新开奖数据
- ✅ 数据直接来自 USA28 官方 API
- ✅ 无需等待数据同步，即时可见

### 2️⃣ 智能降级策略
```
USA28 API (首选)
    ↓ 如果失败
数据库数据 (备用)
```

### 3️⃣ 操作更简单
- 🚫 **移除**：同步开奖数据按钮
- ✅ **新增**：实时数据标识
- ✅ **保留**：刷新数据按钮

---

## 🔧 技术实现

### 后端改进

#### 1. 修改 `getLotteryHistory` 方法

**原来：** 从数据库读取数据
```typescript
async getLotteryHistory(query: QueryLotteryDto) {
  // 从数据库查询...
  return await this.prisma.lotteryResult.findMany({...});
}
```

**现在：** 直接调用 USA28 API
```typescript
async getLotteryHistory(query: QueryLotteryDto) {
  try {
    // 1. 调用 USA28 API
    const response = await axios.get('https://api.365kaik.com/api/v1/trend/getHistoryList', {
      params: { lotCode: '10029', pageSize, pageNum, t: Date.now() },
      httpsAgent: new https.Agent({ rejectUnauthorized: false }),
    });
    
    // 2. 转换数据格式
    const list = response.data.data.list.map(item => ({
      issue: item.drawIssue,
      number1: parseInt(item.drawCode.split(',')[0]),
      number2: parseInt(item.drawCode.split(',')[1]),
      number3: parseInt(item.drawCode.split(',')[2]),
      // ... 计算结果
    }));
    
    return { list, total, page, limit };
    
  } catch (error) {
    // 3. 降级到数据库查询
    return this.getLotteryHistoryFromDB(query);
  }
}
```

#### 2. SSL 证书处理
```typescript
import * as https from 'https';

const httpsAgent = new https.Agent({
  rejectUnauthorized: false, // 忽略证书验证
});
```

### 前端改进

#### 1. 移除同步按钮
```vue
<!-- 原来 -->
<el-button type="primary" @click="handleSync">
  同步开奖数据
</el-button>

<!-- 现在 -->
<el-tag type="success" effect="dark">
  <el-icon><CircleCheck /></el-icon>
  实时数据
</el-tag>
<el-button type="primary" @click="handleRefresh">
  刷新数据
</el-button>
```

#### 2. 简化逻辑
```typescript
// 删除 syncLottery API 调用
// 删除 handleSync 方法

// 刷新数据（直接重新获取）
const handleRefresh = () => {
  ElMessage.info('正在获取最新开奖数据...')
  fetchLotteryHistory()
}
```

---

## 📊 API 数据格式

### USA28 API 返回格式
```json
{
  "code": 0,
  "data": {
    "pageNum": 0,
    "totalCount": 132,
    "pageSize": 20,
    "list": [
      {
        "drawTime": "2025-11-27 04:11:30",
        "drawIssue": "3364685",
        "drawCode": "5,2,1",
        "groupCode": "pc28",
        "result": {
          "pc28_total": "8",
          "pc28_totalBigSmall": "2",
          "pc28_totalOddEven": "2",
          ...
        }
      }
    ]
  },
  "msg": "ok"
}
```

### 系统转换后格式
```json
{
  "list": [
    {
      "id": 3364685,
      "issue": "3364685",
      "number1": 5,
      "number2": 2,
      "number3": 1,
      "resultSum": 8,
      "sizeResult": "小",
      "oddEvenResult": "双",
      "comboResult": "小双",
      "isReturn": 0,
      "returnReason": null,
      "drawTime": "2025-11-27T04:11:30.000Z"
    }
  ],
  "total": 132,
  "page": 1,
  "limit": 20
}
```

---

## 🎯 使用指南

### 查看开奖历史

1. **打开页面**
   - 左侧菜单：点击"开奖历史"
   - 系统自动获取最新数据

2. **刷新数据**
   - 点击"刷新数据"按钮
   - 或者切换页面后返回

3. **搜索期号**
   - 输入期号（如：3364685）
   - 点击"搜索"
   - 支持模糊搜索

### 数据特点

✅ **实时性**
- 数据直接来自 USA28 官方
- 无延迟，即时更新

✅ **准确性**
- 官方数据源，权威可靠
- 自动计算大小、单双、组合、回本

✅ **可靠性**
- API 失败时自动降级到数据库
- 保证系统可用性

---

## ⚙️ 配置说明

### 环境变量（可选）

如果需要配置 USA28 API，可以在 `.env` 中设置：

```env
# USA28 API配置
USA28_API_URL=https://api.365kaik.com/api/v1/trend/getHistoryList
USA28_LOT_CODE=10029
USA28_API_TIMEOUT=30000
```

### 数据库作用

虽然开奖历史不再存储，但数据库仍用于：
- ✅ 用户下注记录
- ✅ 积分变动记录
- ✅ 结算历史
- ✅ API 失败时的降级数据源

---

## 🔍 故障排查

### 问题1：显示"暂无数据"

**可能原因：**
1. USA28 API 无响应
2. 网络连接问题
3. SSL 证书验证失败

**解决方案：**
```bash
# 查看后端日志
cd score-system/backend
npm run start:dev

# 检查是否有错误信息
```

### 问题2：数据显示延迟

**说明：**
- USA28 API 响应时间：1-3秒
- 网络较慢时可能需要更长时间

**优化建议：**
- 添加加载动画（已实现）
- 增加超时时间（默认30秒）

### 问题3：SSL证书错误

**错误信息：**
```
UNABLE_TO_GET_ISSUER_CERT_LOCALLY
```

**解决方案：**
已在代码中处理：
```typescript
const httpsAgent = new https.Agent({
  rejectUnauthorized: false, // 忽略证书验证
});
```

---

## 📈 性能对比

### 旧方案（数据库模式）
```
用户访问 → 数据库查询 → 返回数据
优点：速度快（本地查询）
缺点：需要手动同步，数据可能不是最新
```

### 新方案（实时API模式）
```
用户访问 → USA28 API → 转换数据 → 返回数据
优点：数据永远最新，无需同步
缺点：依赖外部API，速度稍慢（1-3秒）
```

### 智能降级
```
USA28 API (优先) → 数据库 (备用)
兼顾实时性和可靠性
```

---

## 🚀 未来优化

### 1. 缓存策略
```typescript
// 添加短期缓存（如：30秒）
const cache = new Map();

async getLotteryHistory(query) {
  const cacheKey = JSON.stringify(query);
  const cached = cache.get(cacheKey);
  
  if (cached && Date.now() - cached.time < 30000) {
    return cached.data; // 返回缓存
  }
  
  const data = await fetchFromUSA28();
  cache.set(cacheKey, { data, time: Date.now() });
  return data;
}
```

### 2. WebSocket 推送
```typescript
// 实时推送最新开奖
@WebSocketGateway()
export class LotteryGateway {
  @SubscribeMessage('subscribe-lottery')
  handleSubscribe(client: Socket) {
    // 定时推送最新开奖
    setInterval(() => {
      const latest = await getLatestLottery();
      client.emit('lottery-update', latest);
    }, 60000); // 每分钟推送
  }
}
```

### 3. 数据预加载
```typescript
// 后台定时缓存数据
@Cron('0 */5 * * * *') // 每5分钟
async cacheLatestData() {
  const data = await fetchFromUSA28();
  await redis.set('lottery:cache', JSON.stringify(data), 'EX', 300);
}
```

---

## 📞 技术支持

如有问题，请检查：
1. 后端日志（`score-system/backend`）
2. 浏览器控制台（F12 → Console）
3. 网络连接（确保可访问 api.365kaik.com）

---

**更新时间：** 2025-11-27  
**版本：** v2.0 - 实时数据模式  
**作者：** AI Assistant





